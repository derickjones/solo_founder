...
import { searchScriptures, SearchResult } from '@/services/api';
...
export default function ChatInterface({ selectedSources, sourceCount }: ChatInterfaceProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await searchScriptures({
        query: searchQuery,
        mode,
      ...
      const assistantMessage: Message = {
        id: Date.now() + 1,
        type: 'assistant',
        content: `Found ${response.total_found} results for "${searchQuery}" in ${response.search_time.toFixed(2)}s`,
