...
export const generateLessonPlanPDF = async (lessonData: LessonPlanData) => {
  try {
    ...
    pdf.text('Gospel Study App', margin, yPosition);
    yPosition += 10;
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      pdf.text('Generated by Gospel Study App', margin, footerY);
  ...
  try {
    ...
    tempDiv.innerHTML = `
      <div style="margin-bottom: 20px; border-bottom: 2px solid #e5e7eb; padding-bottom: 15px;">
        <h1 style="color: #3b82f6; font-size: 24px; margin: 0 0 10px 0;">Gospel Study App</h1>
      ...
      <div style="margin-top: 30px; padding-top: 15px; border-top: 1px solid #e5e7eb; font-size: 10px; color: #9ca3af;">
        <p>Generated by Gospel Study App on ${new Date().toLocaleDateString()}</p>
      </div>
    `;
